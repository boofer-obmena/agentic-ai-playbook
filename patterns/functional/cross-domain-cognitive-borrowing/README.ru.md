...
- Предшествует [PATTERN-021](../PATTERN-021-agent-debate/) — помогает сформулировать проблему, которая затем выносится на контраст ролей
- Дополняет [PATTERN-017](../PATTERN-017-dynamic-ontology/) — PATTERN-017 исследует свой домен, PATTERN-049 выходит в чужие
- Может инициироваться из [PATTERN-043](../PATTERN-043-single-role-reflective-deepening/) — когда когнитивная рефлексия внутри своего домена исчерпана
- Может инициироваться человеком через [PATTERN-044](../PATTERN-044-evolving-human-agent-trust/)
- Может инициироваться оркестратором через [PATTERN-011](../PATTERN-011-orchestrator/)
- Использует [PATTERN-045](../PATTERN-045-four-level-agent-memory/) — требует доступа к семантической памяти других агентов, включая описание их когнитивных моделей
- Дополняет [METHOD-015](../METHOD-015-guided-exploration/) — METHOD-015 находит пробел, PATTERN-049 может быть использован для его зондирования

**Экспериментальная проверка:** Требуется. План: дать агенту-методологу задачу, для которой в его домене нет прямого решения. Сравнить три режима: (а) только со своей базой знаний, (б) с доступом к базам знаний смежных ролей и описаниям их моделей мышления (инициация оркестратором), (в) с инструкцией на самоинициацию после исчерпания N циклов рефлексии. Измерить: время до формулирования проблемы, качество итогового решения, долю ложных самоинициаций в режиме (в).
